...
import { LayoutDashboard, ClipboardList, Users, KeyRound, LogOut, Menu, X } from 'lucide-react'
import { cn } from '@/lib/utils'
import { useState } from 'react'

const NAV_ITEMS = [
  { href: '/lab-portal/dashboard', label: 'Dashboard', icon: LayoutDashboard },
  { href: '/lab-portal/results',   label: 'Lab Results', icon: ClipboardList },
  { href: '/lab-portal/patients',  label: 'Patients',   icon: Users },
]
...
export default function LabNav({ firstName, lastName }: LabNavProps) {
  const pathname = usePathname()
  const router   = useRouter()
  const [open, setOpen] = useState(false)

  async function logout() {
    await fetch('/api/lab-portal/auth/logout', { method: 'POST' })
    ...
    <nav className="bg-white border-b border-gray-200 sticky top-0 z-50 shadow-sm">
      <div className="max-w-5xl mx-auto px-4">
        <div className="flex items-center h-14 gap-3">
          <Link href="/lab-portal/dashboard" className="flex items-center gap-1 shrink-0 mr-2">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <Link
              href="/lab-portal/change-password"
              className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 text-sm text-gray-500 hover:text-emerald-700 rounded-lg hover:bg-emerald-50 transition-colors"
            >
        ...
        {open && (
          <div className="md:hidden border-t border-gray-100 py-2 pb-3">
            ...
            <Link
              href="/lab-portal/change-password"
              onClick={() => setOpen(false)}
              className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 rounded-lg mx-1 hover:bg-emerald-50"
            >
              <KeyRound className="w-4 h-4" />
              Change Password
            </Link>
            <button
              onClick={logout}
              className="flex items-center gap-3 px-4 py-2.5 text-sm text-emerald-700 rounded-lg mx-1 hover:bg-emerald-50 w-[calc(100%-0.5rem)]"
            >
              <LogOut className="w-4 h-4" />
              Sign out
            </button>
